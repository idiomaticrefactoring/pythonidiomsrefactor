# -*- coding: utf-8 -*-
"""
List Comprehension - Refactor transformation (fully self-contained).

Transforms non-idiomatic for-loop + append patterns into list comprehensions.
"""
...
def _for_transform(node: ast.AST, listcompre: ast.ListComp) -> None:
    """Recursively transform for/if structure into list comprehension."""
    if isinstance(node, ast.For):
        body = node.body[0]
        listcompre.generators.append(_copy_compre(node))
        _for_transform(body, listcompre)
...
def transform(match: Match, config: Dict[str, Any]) -> str:
    ...
    listcompre.elt = None
    _for_transform(for_node, listcompre)
