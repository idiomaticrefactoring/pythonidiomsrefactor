# -*- coding: utf-8 -*-
"""
List Comprehension - Explain transformation (fully self-contained).

Transforms idiomatic list comprehensions back into for-loop + append patterns.
"""
...
def _get_for_str(tmp_var: str, node: ast.ListComp, space: str = "") -> str:
    """Generate for-loop string from list comprehension."""
    code_str = ""
...
def _trans_elt(tmp_var: str, node: ast.AST, space: str) -> str:
    ...
    if isinstance(node, ast.IfExp):
        return (
            ...
            f"{space}    {tmp_var}.append({ast.unparse(node.body)})\n"
        ...
        return (
            ...
            + f"{space}{tmp_var}.append({new_tmp})\n"
        )
    else:
        return f"{space}{tmp_var}.append({ast.unparse(node)})\n"
...
class _RewriteCompre(ast.NodeTransformer):
    """Replace list comprehension with variable reference."""
...
def transform(match: Match, config: Dict[str, Any]) -> str:
    """Transform list comprehension to non-idiomatic for-loop."""
    context = match.context
    parent_node = context["assign_node"]
    node = context["list_comp_node"]
    ...
    if intersect_vars:
        ...
        parent_node.value, ast.ListComp
    ):
