# -*- coding: utf-8 -*-
"""
Set Comprehension - Detection logic (fully self-contained).

Detects:
- detect_refactor: for-loop + add patterns that can become set comprehensions
- detect_explain: existing set comprehensions for explanation
"""
...
def detect_refactor(content: str, config: Dict[str, Any]) -> List[Match]:
    """Detect non-idiomatic for-loop patterns that can become set comprehensions."""
    matches = []
    ...
    try:
        ...
        for tree, class_name in analyzer.func_def_list:
            ...
            code_list = _get_complicated_for_comprehen_code_list(
                tree, const_empty_list=["set()"], const_func_name="add"
            )
...
def detect_explain(content: str, config: Dict[str, Any]) -> List[Match]:
    """Detect idiomatic set comprehension patterns for explanation."""
    matches = []
