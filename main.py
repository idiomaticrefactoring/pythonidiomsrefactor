...
IDIOM_MAPPING: Dict[str, List[Callable]] = {
    "list-comprehension": [
        wrap_idiom_func(list_comp_mod.get_list_compreh, "List Comprehension")
    ],
    "set-comprehension": [
        wrap_idiom_func(set_comp_mod.get_set_compreh, "Set Comprehension")
    ],
    "dict-comprehension": [
        wrap_idiom_func(dict_comp_mod.get_dict_compreh, "Dict Comprehension")
    ],
    "chain-comparison": [
        wrap_idiom_func(chain_compare_mod.get_chain_compare, "Chain Compare")
    ],
    "truth-value-test": [
        wrap_idiom_func(truth_value_mod.get_truth_value_test_code, "Truth Value Test")
    ],
    "for-else": [wrap_idiom_func(for_else_mod.transform_for_else_code, "For Else")],
    "assign-multiple-targets": [
        wrap_idiom_func(
            assign_multi_mod.transform_multiple_assign_code, "Assign Multi Targets"
        )
    ],
    "star-in-func-call": [
        wrap_idiom_func(star_call_mod.transform_star_call_code, "Call Star")
    ],
    "for-multiple-targets": [
        wrap_idiom_func(
            for_multi_target_mod.transform_for_multiple_targets_code,
            "For Multi Targets",
        )
    ],
}
...
def process_single_file(
    ...
    if enable_noqa:
        try:
            ...
            print(
                f"⚠️  Warning: Cannot read {filepath} for noqa check (permission denied)"
            )
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="RefactoringIdioms: Detect and refactor non-idiomatic Python code."
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--filepath", type=str, help="Override target path (file or directory)"
    )
    parser.add_argument(
        "--outputpath", type=str, default="result.json", help="Output JSON file path"
    )
    ...
    # --- 扫描文件 ---
    include_paths = (
        [args.filepath] if args.filepath else tool_conf.get("include", ["."])
    )
