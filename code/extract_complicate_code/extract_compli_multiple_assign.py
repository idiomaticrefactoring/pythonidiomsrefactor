...
def save_repo_for_else_complicated(repo_name):
    ...
    if dict_file:
        ...
        print("it exists for else complicated code")
...
'''
    ...
    pool.join()
    end_time = time.time()
    print("time of extracting all complicated code: ", end_time-beg_time,len(repo_name_list))
    '''
    #'''
...
# import random
    # random.shuffle(result_compli_for_else_list)
    # util.save_csv(util.data_root+"complicated_code_dir/multip_assign_complicated_sample.csv",result_compli_for_else_list[:385],["repo_name","code","file_html","file_path"])
   #'''
